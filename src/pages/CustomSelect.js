...
export default function CustomSelect( {value, listPerPage, onChangeValue} ) {
    console.log('custom select : ', value);
    
    const handleChangePageSize = (target) => {
        console.log("handle change page size : ", target);
        listPerPage(target);
    ...
    return (
        <>
            <Box pl={3}>
                <Box display={"inline-block"} mr={1}>Results : </Box>
                <MultipleSelect value={value} onChange={handleChangePageSize}>
